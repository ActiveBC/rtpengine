...
static void rec_pcap_init(struct call *call) {
	...
	// set up pcap file
	char *rec_pcap_path = recording_setup_file(recording);
	if (rec_pcap_path != NULL && recording->u.pcap.recording_pdumper != NULL
 ...
 */
static void stream_rec_pcap_dump(struct media_packet *mp, const str *s) {
